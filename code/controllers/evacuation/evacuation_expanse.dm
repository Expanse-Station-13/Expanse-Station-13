...
#define EVAC_OPT_HIGH_GRAV "high_grav"
#define EVAC_OPT_CANCEL_ABANDON_SHIP "cancel_abandon_ship"
#define EVAC_OPT_CANCEL_HIGH_GRAV "cancel_high_grav"

// emergency_evacuation --> "abandon ship" and !emergency_evacuation --> "high G"
/datum/evacuation_controller/expanse
	...
	evacuation_options = list(
		...
		EVAC_OPT_HIGH_GRAV = new /datum/evacuation_option/high_grav(),
		EVAC_OPT_CANCEL_ABANDON_SHIP = new /datum/evacuation_option/cancel_abandon_ship(),
		EVAC_OPT_CANCEL_HIGH_GRAV = new /datum/evacuation_option/cancel_high_grav()
	)

/datum/evacuation_controller/expanse/finish_preparing_evac()
	. = ..()
	// Arm the escape pods.
...
/datum/evacuation_controller/expanse/launch_evacuation()
	...
	if(emergency_evacuation)
		...
		SetUniversalState(/datum/universal_state/high_g, arguments=list(GLOB.using_map.station_levels))

/datum/evacuation_controller/expanse/finish_evacuation()
	..()
	if(!emergency_evacuation)
		SetUniversalState(/datum/universal_state)

/datum/evacuation_controller/expanse/available_evac_options()
	if (is_on_cooldown())
		return list()
	if (is_idle())
		return list(evacuation_options[EVAC_OPT_HIGH_GRAV], evacuation_options[EVAC_OPT_ABANDON_SHIP])
	if (is_evacuating())
		if (emergency_evacuation)
			return list(evacuation_options[EVAC_OPT_CANCEL_ABANDON_SHIP])
		else
			return list(evacuation_options[EVAC_OPT_CANCEL_HIGH_GRAV])
...
/datum/evacuation_option/high_grav
	option_text = "Initiate high G maneuver"
	option_desc = "initiate high G maneuver"
	option_target = EVAC_OPT_HIGH_GRAV
	needs_syscontrol = TRUE
	silicon_allowed = TRUE

/datum/evacuation_option/high_grav/execute(mob/user)
	if (!evacuation_controller)
		return
	if (evacuation_controller.deny)
		to_chat(user, "Unable to initiate high G maneuver preparation.")
		return
	if (evacuation_controller.is_on_cooldown())
		to_chat(user, evacuation_controller.get_cooldown_message())
		return
	if (evacuation_controller.is_evacuating())
		to_chat(user, "High G maneuver preparation already in progress.")
		return
	if (evacuation_controller.call_evacuation(user, 0))
		log_and_message_admins("[user? key_name(user) : "Autotransfer"] has initiated high G maneuver preparation.")
...
/datum/evacuation_option/cancel_high_grav
	option_text = "Cancel high G maneuver"
	option_desc = "cancel the high G maneuver preparation"
	option_target = EVAC_OPT_CANCEL_HIGH_GRAV
	needs_syscontrol = TRUE
	silicon_allowed = FALSE

/datum/evacuation_option/cancel_high_grav/execute(mob/user)
	if (evacuation_controller && evacuation_controller.cancel_evacuation())
		log_and_message_admins("[key_name(user)] has cancelled the high G maneuver.")

#undef EVAC_OPT_ABANDON_SHIP
#undef EVAC_OPT_HIGH_GRAV
#undef EVAC_OPT_CANCEL_ABANDON_SHIP
#undef EVAC_OPT_CANCEL_HIGH_GRAV